...
module h5.application {
    ...
    export class AppController {
        ...
        */
        ...
        /**
        * Initialize the ZSYTAB Master module
        */
        private initZSYTABModule() {
...
setFocus() {
                let someElement = window.document.getElementById('AL30');
                someElement.focus();
            };
        ...
        */
        private initScopeFunctions() {
            ...
            this.scope.ZSYTABModule.loadZSYTABRecordModule = (fieldName, rowEntity) => { this.displayDetailScreen(fieldName, rowEntity); }
        }
        ...
        */
        private initUIGridsOnRegisterApi() {
            this.scope.ZSYTABModule.ZSYTABListGrid.onRegisterApi = (gridApi) => {
                this.gridService.adjustGridHeight("ZSYTABListGrid", this.scope.ZSYTABModule.ZSYTABListGrid.data.length, 500);
                ...
                gridApi.selection.on.rowSelectionChanged(this.scope, (row: any) => {
                    ...
                    this.ZSYTABListRowSelected(row);
                });
        ...
        private clearData(categories: string[]) {
            categories.forEach((category) => {
                ...
                if (category == "ZSYTABModule") {
                    ...
                    this.scope.ZSYTABModule.ZSYTABList = [];
                }
            });
        }
        ...
        */
        private refreshTransactionStatus() {
            ...
            switch (this.scope.activeModule) {
                case 1:
                    for (let transaction in this.scope.ZSYTABModule.transactionStatus) {
                        let value = this.scope.ZSYTABModule.transactionStatus[transaction];
                        if (value == true) {
                            isLoading = true;
                            break;
                        }
        ...
        */
        ...
        private loadZSYTABList(): void {
            this.scope.loadingData = true;
            this.scope.ZSYTABModule.transactionStatus.ZSYTABList = true;
            this.scope.ZSYTABModule.ZSYTABRecord.PK01 = "";
            this.appService.lstZSYTABRecords().then((val: M3.IMIResponse) => {
                let filteredVal = this.filterZSYTAB("ZSYTAB", val);
                this.scope.ZSYTABModule.ZSYTABList = filteredVal;
                this.scope.ZSYTABModule.ZSYTABListGrid.data = filteredVal;//val.items; //added after adding the list

                this.gridService.adjustGridHeight("ZSYTABListGrid", filteredVal.length, 1000) //added to adjust grid height
                this.scope.ZSYTABModule.transactionStatus.ZSYTABList = false;
                this.refreshTransactionStatus();
            }, (err: M3.IMIResponse) => {
                this.scope.ZSYTABModule.transactionStatus.ZSYTABList = false;
                this.refreshTransactionStatus();
        ...
        private loadZSYTABRecord(fieldName: string, selectedRow): void {
            this.scope.loadingData = true;
            this.scope.ZSYTABModule.transactionStatus.ZSYTABRecord = true;
            this.appService.getZSYTABRecord(selectedRow.PK01, selectedRow.PK02, selectedRow.PK03).then((val: M3.IMIResponse) => {
                this.scope.ZSYTABModule.ZSYTABRecord = val.items[0];
        ...
        public saveButtonOnClick(): void {
            this.scope.ZSYTABModule.transactionStatus.ZSYTABRecord = true;
            // Needed keys
            let tableName = this.scope.ZSYTABModule.ZSYTABRecord.PK01;
            let tableDesc = this.scope.ZSYTABModule.ZSYTABRecord.AL30;
            let path = this.scope.ZSYTABModule.ZSYTABRecord.AL31;
            let server = this.scope.ZSYTABModule.ZSYTABRecord.AL32;
            let folderPath = this.scope.ZSYTABModule.ZSYTABRecord.AL33;
            let port = this.scope.ZSYTABModule.ZSYTABRecord.AL34;
            let protocol = this.scope.ZSYTABModule.ZSYTABRecord.AL35;
            //fileds to save
            this.saveZSYTABRecord(tableName, tableDesc, path, server, folderPath, port, protocol);
           
        }
        public saveZSYTABRecord(tableName: string, tableDesc: string, path: string, server: string, folderPath: string, port: string, protocol: string): void {
            this.scope.ZSYTABModule.transactionStatus.ZSYTABRecord = true;
            this.appService.saveZSYTABRecord(tableName, tableDesc, path, server, folderPath, port, protocol).then((val: M3.IMIResponse) => {
                //   (tableName: string, tableDesc: string, path: string,server: string, folderPath: string, port: string, protocol: string)
                this.scope.ZSYTABModule.transactionStatus.ZSYTABRecord = false;
                this.refreshTransactionStatus();
                 this.loadZSYTABList();
            this.closeModalWindow();
          ...
          public myError(err: M3.IMIResponse): void {
            ...
            this.refreshTransactionStatus();
            let error = "API: " + err.program + "." + err.transaction + ", Input: " + JSON.stringify(err.requestData) + ", Error Code: " + err.errorCode;
            //this.showMyError(error, [err.errorMessage]);
        ...
        private openWindow() {
            ...
            let appName = "";
            // get record
            let selectedZSYTABTable = this.scope.ZSYTABModule.ZSYTABRecord.PK01;
            this.scope.loadingData = true;
        ...
        public addRecord(): void {
            ...
            if (tableName == "" || tableName == null) {
                let error = "Table Name must be entered";
                this.showError(error, [error]);
            }
            else {this.appService.addZSYTABRecord(tableName, tableDesc, path, server, folderPath, port, protocol).then((val: M3.IMIResponse) => {
        ...
        public deleteRecord(): void {
            this.scope.ZSYTABModule.transactionStatus.ZSYTABRecord = true;
            // Needed keys
            let keyVal = this.scope.ZSYTABModule.ZSYTABRecord.PK01;//constant

            let language = this.scope.ZSYTABModule.ZSYTABRecord.PK02;
            let divi = this.scope.ZSYTABModule.ZSYTABRecord.PK03;




            this.appService.deleteCSYTABRecord(keyVal, language, divi).then((val: M3.IMIResponse) => {
                this.loadZSYTABList();
                this.closeModalWindow();
                this.scope.ZSYTABModule.transactionStatus.ZSYTABRecord = false;
                this.refreshTransactionStatus();
        ...
        public openAddRecordwindow(): void {
            ...
            if (tableName == "" || tableName == null || tableName == 0) {
                ...
                this.appService.getAlphaZSYTABRecord(tableName).then((val: M3.IMIResponse) => {
                    if (val.errorField === undefined) {
                        this.scope.ZSYTABModule.ZSYTABRecord.PK01 = "";
                        let error = tableName + " Table already exists";
                        this.showError(error, null);
                    }

                }, (err: M3.IMIResponse) => {
                    this.scope.ZSYTABModule.ZSYTABRecord.KPID = "ZSYTAB";
                    // this.scope.ZSYTABModule.ZSYTABRecord.PK01 = "";
                    this.scope.ZSYTABModule.ZSYTABRecord.PK02 = "";
                    this.scope.ZSYTABModule.ZSYTABRecord.AL30 = "";
                    this.scope.ZSYTABModule.ZSYTABRecord.AL31 = "";
                    this.scope.ZSYTABModule.ZSYTABRecord.AL32 = "";
                    this.scope.ZSYTABModule.ZSYTABRecord.AL33 = "";
                    this.scope.ZSYTABModule.ZSYTABRecord.AL34 = "";
                    this.scope.ZSYTABModule.ZSYTABRecord.AL35 = "";
                    this.scope.ZSYTABModule.ZSYTABRecord.AL36 = "";
                    this.openZSYTABAddRecordModal();
        ...
        */
        private loadZSYTABModule(reLoad: boolean): void {
            let userContext = this.scope.userContext;
        ...
        private openZSYTABAddRecordModal(): void { //POPUP
            let options: any = {
                animation: true,
        ...
        private ZSYTABListRowSelected(selectedRow: any) {
